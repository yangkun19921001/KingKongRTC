...
#include "video/nv12_buffer.h"

#include "video/i420_buffer.h"
...
// static
kkrtc::scoped_refptr<NV12Buffer> NV12Buffer::Create(int width, int height) {
  return kkrtc::make_ref_counted<NV12Buffer>(width, height);
}

// static
kkrtc::scoped_refptr<NV12Buffer> NV12Buffer::Create(int width,
                                                  int height,
                                                  int stride_y,
                                                  int stride_uv) {
  return kkrtc::make_ref_counted<NV12Buffer>(width, height, stride_y, stride_uv);
}

// static
kkrtc::scoped_refptr<NV12Buffer> NV12Buffer::Copy(
    const I420BufferInterface& i420_buffer) {
  kkrtc::scoped_refptr<NV12Buffer> buffer =
...
kkrtc::scoped_refptr<I420BufferInterface> NV12Buffer::ToI420() {
