...
namespace kkrtc {
 ...
 /*       ////RTC_DCHECK_GE(width, 0);
        ...
        switch (type) {
            case KKVideoFormat::I420:
            case KKVideoFormat::IYUV:
            case KKVideoFormat::YV12: {
                int half_width = (width + 1) >> 1;
                int half_height = (height + 1) >> 1;
                buffer_size = width * height + half_width * half_height * 2;
                break;
            }
            case KKVideoFormat::RGB565:
            case KKVideoFormat::YUY2:
            case KKVideoFormat::UYVY:
                buffer_size = width * height * 2;
                break;
            case KKVideoFormat::RGB24:
    ...
    int ExtractBuffer(const kkrtc::scoped_refptr<I420BufferInterface>& input_frame,
        ...
        size_t length = CalcBufferSize(KKVideoFormat::I420, width, height);
        if (size < length) {
            return -1;
        }
    ...
    int ConvertKKVideoFormat(KKVideoFormat video_type) {
        switch (video_type) {
            case KKVideoFormat::Unknown:
                return libyuv::FOURCC_ANY;
            case KKVideoFormat::I420:
                return libyuv::FOURCC_I420;
            case KKVideoFormat::IYUV:  // same as KKVideoFormat::YV12
            case KKVideoFormat::YV12:
    ...
    int ConvertFromI420(const VideoFrame& src_frame,
                        ...
                        int dst_sample_size,
                        uint8_t* dst_frame) {
        kkrtc::scoped_refptr<I420BufferInterface> i420_buffer =
        ...
        return libyuv::ConvertFromI420(
                ...
                ConvertKKVideoFormat(dst_video_type));
    }

    kkrtc::scoped_refptr<I420ABufferInterface> ScaleI420ABuffer(
            const I420ABufferInterface& buffer,
            int target_width,
            int target_height) {
        kkrtc::scoped_refptr<I420Buffer> yuv_buffer =
                I420Buffer::Create(target_width, target_height);
        yuv_buffer->ScaleFrom(buffer);
        kkrtc::scoped_refptr<I420Buffer> axx_buffer =
                I420Buffer::Create(target_width, target_height);
        libyuv::ScalePlane(buffer.DataA(), buffer.StrideA(), buffer.width(),
                           ...
                           libyuv::kFilterBox);
        kkrtc::scoped_refptr<I420ABufferInterface> merged_buffer = WrapI420ABuffer(
    ...
    kkrtc::scoped_refptr<I420BufferInterface> ScaleVideoFrameBuffer(
            ...
            int dst_width,
            int dst_height) {
        kkrtc::scoped_refptr<I420Buffer> scaled_buffer =
...
// Compute PSNR for an I420A frame (all planes). Can upscale test frame.
    double I420APSNR(const I420ABufferInterface& ref_buffer,
        ...
        if ((ref_buffer.width() != test_buffer.width()) ||
            ...
            kkrtc::scoped_refptr<I420ABufferInterface> scaled_buffer =
