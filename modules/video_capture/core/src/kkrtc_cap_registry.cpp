 ...
 */
        class VideoCaptureBackendRegistry {
        protected:
            std::vector<VideoCapBackendInfo> enabledBackends;

            VideoCaptureBackendRegistry() {
                ...
                if (readPrioritySettings()) {
                    KKLogDebugTag("kkrtc_cap_registry") << "VIDEO CAP: Updated backends priorities: " << dumpBackends();
                }
                int enabled = 0;
                for (int i = 0; i < N; i++) {
        ...
        public:
            ...
            static VideoCaptureBackendRegistry &getInstance() {
                static VideoCaptureBackendRegistry g_instance;
                return g_instance;
            }
    ...
    namespace vcap_registry {
        ...
        std::vector<VideoCapBackendInfo> getAvailableBackends_CaptureByIndex() {
            const std::vector<VideoCapBackendInfo> result = VideoCaptureBackendRegistry::getInstance().getAvailableBackends_CaptureByIndex();
            return result;
        }

        std::vector<VideoCapBackendInfo> getAvailableBackends_CaptureByFilename() {
            const std::vector<VideoCapBackendInfo> result = VideoCaptureBackendRegistry::getInstance().getAvailableBackends_CaptureByFilename();
            return result;
        }
    ...
    for (size_t i = 0; i < M; i++)
    {
        ...
        std::vector<KKVideoCaptureAPIs> getBackends() {
            std::vector<VideoCapBackendInfo> backends = VideoCaptureBackendRegistry::getInstance().getEnabledBackends();
        ...
        std::vector<KKVideoCaptureAPIs> getCameraBackends() {
            const std::vector<VideoCapBackendInfo> backends = VideoCaptureBackendRegistry::getInstance().getAvailableBackends_CaptureByIndex();
            std::vector<KKVideoCaptureAPIs> result;
            for (size_t i = 0; i < backends.size(); i++)
                result.push_back((KKVideoCaptureAPIs) backends[i].id);
            return result;
        ...
        std::vector<KKVideoCaptureAPIs> getStreamBackends() {
            const std::vector<VideoCapBackendInfo> backends = VideoCaptureBackendRegistry::getInstance().getAvailableBackends_CaptureByFilename();
            std::vector<KKVideoCaptureAPIs> result;
            for (size_t i = 0; i < backends.size(); i++)
                result.push_back((KKVideoCaptureAPIs) backends[i].id);
            return result;
